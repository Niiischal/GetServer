...
router.post('/',strictVerifyToken, async (req, res) => {
    ...
    try{
        ...
        if(book && book.bookTitle === body.bookTitle && JSON.stringify(book.userId) === JSON.stringify(userId) && book.newBook === body.newBook && book.price === body.price){
        ...
        res.status(201).send({message:'Book added successfully.', data: data});
    }
    catch(error){
